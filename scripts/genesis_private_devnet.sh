...
byxd genesis add-genesis-account "${GENESIS_ADDR}" "${GENESIS_ACCOUNT_BYX}byx" --home "${HOME_DIR}" --keyring-backend "${KEYRING_BACKEND}" >/dev/null
byxd genesis add-genesis-account "${VALIDATOR_ADDR}" "${VALIDATOR_SELF_DELEGATION_BYX}byx" --home "${HOME_DIR}" --keyring-backend "${KEYRING_BACKEND}" >/dev/null
byxd genesis add-genesis-account "${LOJAS_MODULE_ADDR}" "${LOJAS_RESERVE_BYX}byx" --home "${HOME_DIR}" >/dev/null

echo "==> Gentx + collect-gentxs"
byxd genesis gentx "${VALIDATOR_KEY_NAME}" "${VALIDATOR_SELF_DELEGATION_BYX}byx" --chain-id "${CHAIN_ID}" --home "${HOME_DIR}" --keyring-backend "${KEYRING_BACKEND}" >/dev/null 2>&1
...
jq \
  ...
  --argjson total_supply "${TOTAL_SUPPLY_BYX}" \
  '
  .chain_id = $chain_id
  | .app_state.staking.params.bond_denom = "byx"
  | .app_state.gov.params.min_deposit = [{"denom":"byx","amount":"10000000"}]
  | .app_state.gov.params.expedited_min_deposit = [{"denom":"byx","amount":"50000000"}]
  | .app_state.mint.minter.inflation = "0.000000000000000000"
  | .app_state.mint.params.mint_denom = "byx"
  ...
  | .app_state.bank.supply |= (map(if .denom == "byx" then .amount = ($total_supply|tostring) else . end))
