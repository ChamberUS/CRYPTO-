...
func newTestnetApp(
	...
	// Cria a App normalmente
	a := app.New(logger, db, traceStore, true, appOpts)

	// Pega os args do comando
	args, err := getCommandArgs(appOpts)
	if err != nil {
		panic(err)
	}

	// Ajusta o estado para o modo testnet e retorna (*app.App implementa servertypes.Application)
	return initAppForTestnet(a, args)
}
